          ...
          <div className="divide-y divide-gray-50">
            {events.slice(0, 5).map((event) => (
              <div key={event.id} className="px-6 py-4 flex items-center justify-between hover:bg-gray-50 transition-colors">
                <div>
                  <Link to={`/e/${event.visibility === 'private' ? event.privateToken : (event.slug || event.id)}`} className="font-medium text-gray-900 text-sm hover:text-brand-600 hover:underline">{event.title}</Link>
